...
def _hex2rgb(h:str): h=h.lstrip("#"); return int(h[0:2],16),int(h[2:4],16),int(h[4:6],16)
def _mk(stops: List[Tuple[int,str]]):
    lut=np.zeros((256,3),np.uint8)
    for (p0,c0),(p1,c1) in zip(stops[:-1],stops[1:]):
        r0,g0,b0=_hex2rgb(c0); r1,g1,b1=_hex2rgb(c1); span=max(1,p1-p0)
    ...
    p,c=stops[-1]; lut[p:]=np.array(_hex2rgb(c),np.uint8); return lut
_LUTS: Dict[str,np.ndarray]={}
def _lut(name:str)->np.ndarray:
    k=name.lower()
    if k in _LUTS: return _LUTS[k]
    maps={"grayscale":[(0,"#000000"),(255,"#ffffff")],
    ...
    _LUTS[k]=_mk(maps.get(k,maps["grayscale"])); return _LUTS[k]
def _apply(gray: np.ndarray, cmap: str)->np.ndarray:
    if gray.ndim==3 and gray.shape[2]==3: return gray
    if gray.dtype!=np.uint8:
        f=gray.astype(np.float32); mn,mx=np.nanmin(f),np.nanmax(f)
        f=np.clip(f,0,1) if (not np.isfinite(mn) or not np.isfinite(mx) or mx<=mn) else np.clip((f-mn)/(mx-mn),0,1)
        gray=(f*255+0.5).astype(np.uint8)
    return _lut(cmap.lower())[gray]

def _even_dims(h:int,w:int)->tuple[int,int]: return (math.ceil(h/2)*2, math.ceil(w/2)*2)
def _even_frame(a: np.ndarray)->np.ndarray:
    h,w=a.shape[:2]; hp,wp=_even_dims(h,w)
    if (hp,wp)==(h,w): return a
    out=np.zeros((hp,wp,3),dtype=np.uint8) if a.ndim==3 else np.zeros((hp,wp),dtype=a.dtype); out[:h,:w]=a; return out
def _pad_w(img: np.ndarray, width:int)->np.ndarray:
    ...
    out=np.zeros((h,width),img.dtype) if img.ndim==2 else np.zeros((h,width,img.shape[2]),img.dtype); out[:,:w]=img; return out
def _vstack_same_w(rows: Sequence[np.ndarray])->np.ndarray:
    maxw=max(r.shape[1] for r in rows); return np.vstack([_pad_w(r,maxw) for r in rows])
def _resize_h(frame: np.ndarray, target_h:int)->np.ndarray:
    if target_h<=0 or frame.shape[0]==target_h: return frame
    scale=target_h/float(frame.shape[0]); new_w=max(2,int(round(frame.shape[1]*scale)))
    from PIL import Image as _I
    return np.array(_I.fromarray(frame).resize((new_w,target_h),resample=_I.NEAREST))

def _split_mid(row: np.ndarray): h,w=row.shape[:2]; mid=max(1,w//2); return row[:,:mid], row[:,mid:], mid
def _auto_split_valley(row: np.ndarray) -> Optional[Tuple[np.ndarray,np.ndarray,int]]:
    rowm=row.mean(axis=2) if row.ndim==3 else row
    col=rowm.mean(axis=0).astype(np.float32)
    if col.size<64: return None
    k=max(7,col.size//150); sm=np.convolve(col,np.ones(k)/k,mode="same")
...
def _fit_half_width(img: np.ndarray, half_w:int)->np.ndarray:
    ...
    out=np.zeros((h,half_w),img.dtype) if img.ndim==2 else np.zeros((h,half_w,img.shape[2]),img.dtype); out[:,:w]=img; return out
...
def _phase_corr_shift(L: np.ndarray, R: np.ndarray, max_shift:int)->Tuple[int,float]:
    try:
        ...
        A = L[band, -min(128,wL):].astype(np.float32); B = R[band, :min(128,wR)].astype(np.float32)
...
def _compose_window(rows: Sequence[np.ndarray], cfg: Dict[str,Any], mode:str, show_seam:bool, smooth_shift:int, shift_hist:List[int], state: Dict[str,Optional[int]], edge_pad:int=0):
    ...
    if edge_pad>0:
        h,w=frame.shape[:2]; out=np.zeros((h,w+edge_pad*2,3),np.uint8); out[:,edge_pad:edge_pad+w]=frame; frame=out
    return frame, seam, shift, score

def _rows_needed(video_h:int, row_h:int)->int: return max(1, int(math.ceil(video_h / max(1,row_h))))

def build_preview_frame(row_paths: Sequence[str], cfg: Dict[str, Any], row_h:int, video_h:int):
    cmap=str(cfg.get("COLORMAP","amber")); mode=str(cfg.get("PREVIEW_MODE","compose")).lower()
    show_seam=bool(cfg.get("SHOW_SEAM", False)); smooth=int(cfg.get("SMOOTH_SHIFT",11)); edge_pad=int(cfg.get("EDGE_PAD",0))
    need=_rows_needed(video_h,row_h); window=[np.array(Image.open(p)) for p in row_paths[:need]]
    if not window: raise RuntimeError("No rows for preview")
    state={"HALF_W":None}; gray,seam,shift,score=_compose_window(window, cfg, mode, show_seam, smooth, [], state, edge_pad=edge_pad)
...
def export_waterfall_mp4(row_paths: Sequence[str], cfg: Dict[str, Any], out_mp4: str,
    ...
    cmap=str(cfg.get("COLORMAP","amber")); smooth=int(cfg.get("SMOOTH_SHIFT",11)); edge_pad=int(cfg.get("EDGE_PAD",0))
