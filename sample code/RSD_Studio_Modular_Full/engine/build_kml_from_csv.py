...
def main():
    ...
    ap.add_argument("--seq-start", type=int, default=None); ap.add_argument("--seq-end", type=int, default=None)
    a=ap.parse_args()
    pts=[]
    ...
    if not pts: raise SystemExit("No lat/lon points.")
    k=['<?xml version="1.0" encoding="UTF-8"?>','<kml xmlns="http://www.opengis.net/kml/2.2"><Document>',
       f'<Placemark><name>{Path(a.csv).stem}</name><LineString><tessellate>1</tessellate><coordinates>']
    for lon,lat in pts: k.append(f"{lon},{lat},0")
    k+=['</coordinates></LineString></Placemark></Document></kml>']
